...
def do_plasim_locked(name,vws,nang,pCO2,p0,flux,lons,lats): #data,lats,lons,pCO2,p0,flux,grav=9.80665
  ...
  jobscript += ("cp "+workdir+"/sbout* /mnt/scratch-lustre/paradise/crawler2/sbdart_locked/%s/     \n"%name+
                "rm -rf "+workdir+"/ \n")
  
  with open("/mnt/scratch-lustre/paradise/crawler2/sbdart_locked/%s/fixsbdart.sh"%name,"w") as rf:
      rf.write(jobscript)
  os.system("chmod a+x /mnt/scratch-lustre/paradise/crawler2/sbdart_locked/%s/fixsbdart.sh"%name)
  ...
  for jlon in lons:
    ...
    #vws = ['Z','N','E','S','W']
    nv = viewdict[vws[0]]
    write_input_locked(workdir+"/sbdart-%02d_%02d_%1d_%s"%(jlat,jlon,nang,vws[0]),
  ...
  os.system("bash /mnt/scratch-lustre/paradise/crawler2/sbdart_locked/%s/fixsbdart.sh"%name) 

  
   
def getbroken(name):
    namedir = "/mnt/scratch-lustre/paradise/crawler2/sbdart_locked/%s"%name 
    ...
    for b in broken:
        ...
        if parts[0]!='32' and parts[1]!='64':
            ...
            os.system("rm /mnt/scratch-lustre/paradise/crawler2/sbdart_locked/%s/%s"%(name,b))
...
if __name__=="__main__":
    ...
    if mode=="PREP":
        ...
        for l in llist:
            if l!="":
                ...
                runscript = ("#!/bin/bash -l                                                 \n"+
                             ...
                             "python fixsbdart_locked.py RUN %s %s %s %s \n"%(name,pco2,flux,p0))
        ...
        do_plasim_locked(name,(view,),nang,pco2,p0,flux,lons,lats)
    #submit(job)