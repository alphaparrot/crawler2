...
def submit(job):
  ...
  if "DEPENDENCIES" in job.parameters:
      ...
      os.system("cd "+workdir+" && "+HOLD(priorjobs)+" runsbdart_earth > %s/%s.id && cd "%(job.top,job.pid)+job.top)
  else:
      os.system("cd "+workdir+" && "+SUB+" runsbdart_earth > %s/%s.id && cd "%(job.top,job.pid)+job.top)