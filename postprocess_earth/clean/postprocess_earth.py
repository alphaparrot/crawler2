...
def collect(lats,lons,times,angles,imxz,modelfile="spectra.nc"):
  ...
  for nln in range(0,3*nlons/4):
      try:
        ...
        raise IndexError("Problem accessing sbout.%02d_%02d_%1d_%s: index out of range"%(nlats/2,nln,times[0],angles[0]))
      if bundle["type"]=="tbf":
        p = bundle["phis"]
        z = bundle["zens"]
        w = bundle["wvl"]
        ...
        for jlon in range(0,nlons):
          ...
          try:
              ...
              try:
                ...
                raise IndexError("Error accessing sbout.%02d_%02d_%1d_%s: index out of range."%(jlat,jlon,kt,angles[ka]))
              if bundle["type"]=="tbf":
                phis = bundle["phis"]
                zens = bundle["zens"]
                wvls = bundle["wvl"]
