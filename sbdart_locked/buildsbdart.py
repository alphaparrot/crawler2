...
def par_do_plasim_locked(job,vws,nang,lons,lats):
  name = job[0]
  with open("../.home","r") as homef:
    top = homef.read().split('\n')[0]
...
def batch_plasim_locked(jid,job):
  ...
  for jlon in lons:
    ...
    nang = nangs[n]
    n+=1
    csz,azm,surf,sic,tsurf,altz = analyzecell_plasim_locked(data,[vw,],jlat,jlon,
...
def do_plasim_locked(job,vws,nang,lons,lats):
  name = job[0]
  ...
  if len(lons)>0:
      do_plasim_locked(job,views,nang,lons,lats)   #Recursive--risky, but this should run
                                                   #we're all good.
...
if __name__=="__main__":
    ...
    if mode=="crawler2":
        ...
        name = job[0]
        lons,lats,nang,views = getbroken(job)
        do_plasim_locked(job,views,nang,lons,lats)
    elif mode=="parfixup":
        name = job[0]
        lons,lats,nang,views = getbroken(job)
        par_do_plasim_locked(job,views,nang,lons,lats)
    elif mode=="batchfix":
        name = job[0]
