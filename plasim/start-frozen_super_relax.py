...
if __name__=="__main__":
  ...
  if not stoprun:
    ...
    minyears=50
    relaxed=False
    while (year < minyears or not energybalanced(threshhold=4.0e-4)) and (cyear<maxyears) and (time.clock()-tstart)<=TIMELIMIT:
      year+=1
      cyear+=1
    ...
    if not energybalanced(threshhold=4.0e-4):
      os.system("touch keepgoing")
      tempt = gethistory()
      with open("thistory.pso","a+") as f:
          text = '\n'+'\n'.join(tempt.astype(str))
          f.write(text)
