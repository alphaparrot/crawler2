...
if __name__=="__main__":
    ...
    ugascon=None
    output = sys.argv[1]
    xH2 = float(sys.argv[2])
    xHe = float(sys.argv[3])
    xCO2 = float(sys.argv[4])
    xN2 = float(sys.argv[5])
    xO2 = float(sys.argv[6])
    try:
        ugascon = float(sys.argv[7])
    except:
        ugascon = None
    ...
    for lon in [16,48]:
        for lat in range(len(lt)):
            ...
            for nt in ntimes:
                ...
                snaptransits[nt,:] += atmosphere.transm_rad*dlat[lat]
                nx+=1
                latwt += dlat[lat]
    transit /= latwt
    snaptransits /= (latwt/float(nt))
