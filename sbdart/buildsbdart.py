...
def analyzecell_plasim(data,lat,lon,workdir,grav=9.80665,smooth=False,clouds=True):
  ...
  altz = data.variables["netz"][-1,lat,lon]*1.0e-3 / grav #kilometers
...
def _prep_lmdz(job):    
  ...
  os.system("cp -r "+job.top+"/sbdart/release.sh "+workdir+"/")
  os.system("cp -r "+job.top+"/sbdart/checkprogress.py "+workdir+"/")
...
def _prep_plasim_earth(job): #data,lats,lons,pCO2,p0,flux,grav=9.80665
  ...
  os.system("cp -r "+job.top+"/sbdart/release.sh "+workdir+"/")
  os.system("cp -r "+job.top+"/sbdart/checkprogress_earth.py "+workdir+"/")
  os.system("cp "+job.top+"/crawldefs.py "+workdir+"/")
  os.system("cp "+job.top+"/identity.py "+workdir+"/")
