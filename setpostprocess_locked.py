...
def submit(job):
    ...
    if "DEPENDENCIES" in job.parameters:
        ...
        os.system("cd "+homedir+" && "+HOLD(priorjobs)+" runpostprocess_locked > %s/%d.id && cd ../../"%(job.top,job.pid))
    else:
        os.system("cd "+homedir+" && "+SUB+" runpostprocess_locked > %s/%d.id && cd ../../"%(job.top,job.pid))
