...
c
      ...
      if(wl.lt.wlalb(1) .or. wl.gt.wlalb(nna))
     &     call errmsg(18,'SALBEDO--spectral range exceeded')
...
c bug correction: remove the following three lines
c      if(isalb.eq.-1) then
c        return
c      endif
...
c bug correction:
c     f2=f2-1./cos(vzap)+.5*(1.+cossp)/(cos(szap)*cos(vzap))
...
c       28, 1714-1719, 1989, with some updated data for the IR)
     ...
     &  4.485e-01, 4.434e-01, 4.383e-01, 4.176e-01, 4.167e-01/
      ...
      wt=max(0.,min(1.,wt))     ! no extrapolation
...
c absolute value for a1 to get v=0 when ni=0
      ...
      b2=2*nr*ni*coschi
c bug correction
c     rl2=((b1-u)**2+(b2+v)**2)/((b1+u)**2+(b2-v)**2)
...
c
 ...
 10   continue
      ...
      if(nn.gt.nnsv) then
        print *,'ERROR in rdspec --- too many values read from ',file
