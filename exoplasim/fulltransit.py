...
if __name__=="__main__":
    ...
    if len(sys.argv)==1:
        ...
        for output in outputs:
            ...
            for lon in [16,48]:
                for lat in range(len(lt)):
                    for nt in ntimes:
                        ...
                        new_q[:50] = h2o_spechum_sim[0]*(new_p[:50]/p_sim[0])**m1
                        new_q[50:98] = h2o_spechum_sim[1]*(new_p[50:98]/p_sim[1])**m1
                        ...
                        R_pl = 2.64* nc.r_earth 
                        gravity = 13.93 * 1e2 # m to cm
                        ...
                        if 'n2' in prefix:
                            abundances = {}
                            abundances['H2'] = 0.0 * np.ones_like(temperature)
                            abundances['He'] = 0.0 * np.ones_like(temperature)
                            abundances['N2'] = 0.99 * np.ones_like(temperature)
                            #abundances['H2O'] = h2o_vmr_sim * 20. / 2.33 #h2o_spechum_sim #*1e10
                            #abundances['H2O'] = h2o_spechum_sim
                            abundances['H2O'] = new_q
                            #abundances['CO_all_iso'] = 0.0 * np.ones_like(temperature)
                            abundances['CO2'] = 0.0 * np.ones_like(temperature)
                            ...
                            MMW = 287.0 * np.ones_like(temperature)
                        elif '_mx_' in prefix:
                            abundances = {}
                            abundances['H2'] = 0.9*0.75 * np.ones_like(temperature)
                            abundances['He'] = 0.9*0.25 * np.ones_like(temperature)
                            abundances['N2'] = 0.08 * np.ones_like(temperature)
                            #abundances['H2O'] = h2o_vmr_sim * 20. / 2.33 #h2o_spechum_sim #*1e10
                            #abundances['H2O'] = h2o_spechum_sim
                            abundances['H2O'] = new_q
                            #abundances['CO_all_iso'] = 0.0 * np.ones_like(temperature)
                            ...
                            abundances['H2O(c)'] = new_cloud*new_q
                            #abundances['H2O(c)'][1] = 1e6*abundances['H2O'][1]
                            MMW = 4.95 * np.ones_like(temperature)
                        elif '_hmx_' in prefix:
                            abundances = {}
                            abundances['H2'] = 0.25*0.75 * np.ones_like(temperature)
                            abundances['He'] = 0.25*0.25 * np.ones_like(temperature)
                            abundances['N2'] = 0.73 * np.ones_like(temperature)
                            #abundances['H2O'] = h2o_vmr_sim * 20. / 2.33 #h2o_spechum_sim #*1e10
                            #abundances['H2O'] = h2o_spechum_sim
                            abundances['H2O'] = new_q
                            #abundances['CO_all_iso'] = 0.0 * np.ones_like(temperature)
                        ...
                        transit += atmosphere.transm_rad*dlat[lat]
            ...
            plt.plot(nc.c/atmosphere.freq/1e-4, (transit/(0.46*6.9551e10))**2*1e6, label = 'cloudy', zorder = 1)
            #plt.xlim(0.6,5.0)
            plt.xlim(0.4,6.0)
            ...
            fig,ax=plt.subplots(figsize=(10,6))
            for nnx in range(nx):
                plt.plot(nc.c/atmosphere.freq/1e-4, (alltransits[nnx,:]/1e5), alpha=0.4,color='k', zorder = 1)
            #plt.xlim(0.6,5.0)
            plt.xlim(0.4,6.0)
            # For the minor ticks, use no labels; default NullFormatter.
            #plt.xlim(1.0e-3,1.0e2)
            ...
            for lat in range(len(lt)):
                ...
                for nt in ntimes:
                    ...
                    if 'n2' in prefix:
                        ...
                        xn2 = float(prefix.split('_m')[1].split('_')[0])
                        gasesx={}
                        gasesx['H2'] = (1-xn2)*0.75*(1-new_q)
                        gasesx['He'] = (1-xn2)*0.25*(1-new_q)
                        gasesx['CO2'] = 1.0e-2*(1-new_q)
                        gasesx['N2'] = (xn2 - 1.0e-2)*(1-new_q)
                        gasesx['H2O'] = new_q
                        abundances = {}
                        abundances['H2'] = (1-xn2)*0.75 * np.ones_like(temperature)
                        abundances['He'] = (1-xn2)*0.25 * np.ones_like(temperature)
                        abundances['N2'] = (xn2-1.0e-2) * np.ones_like(temperature)
                            #abundances['H2O'] = h2o_vmr_sim * 20. / 2.33 #h2o_spechum_sim #*1e10
                            #abundances['H2O'] = h2o_spechum_sim
                        abundances['H2O'] = new_q
                        #abundances['CO_all_iso'] = 0.0 * np.ones_like(temperature)
                        abundances['CO2'] = 1.0e-2 * np.ones_like(temperature)
                        #abundances['CH4'] = 0.0 * np.ones_like(temperature)
                        #abundances['Na'] = 0.0 * np.ones_like(temperature)
                        #abundances['K'] = 0.0 * np.ones_like(temperature)
                            #abundances['H2O(c)'] = 0.0 * np.ones_like(temperature)
